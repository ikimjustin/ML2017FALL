...
from keras.layers import Input, GRU, LSTM, Dense, Dropout, Bidirectional, Flatten, Dot, Add, Concatenate
...
print('initial model...')
def dnnmodel(users, items, dim, dropout_rate):
	user = Input(shape=[1])
	item = Input(shape=[1])
	
	user_vec = Embedding(users, dim, embeddings_initializer='random_normal')(user)
	user_vec = Flatten()(user_vec)
	item_vec = Embedding(items, dim, embeddings_initializer='random_normal')(item)
	item_vec = Flatten()(item_vec)
	...
	model = Dense(dim, activation='relu')(model)
	model = Dropout(dropout_rate)(model)
	model = Dense(1, activation='linear')(model)
	model =keras.models.Model([user, item], model)
...
if sys.argv[1] == "train":
	...
	checkpoint = ModelCheckpoint("dnnbest_model", verbose = 1 ,save_best_only = True, monitor = 'val_loss')
...
if sys.argv[1] == "test":
	...
	model.load_weights('dnnbest_model')	
	...
	for i, j in enumerate(output):
		csv.writer(file).writerow([str(i+1), str((j[0]))])
	file.close()
